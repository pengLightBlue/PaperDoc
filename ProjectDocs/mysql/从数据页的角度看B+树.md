...
在 InnoDB 聚簇索引的非叶子节点中：

- **存储的是物理页号（page number）**，而不是内存指针
- 这个页号是 InnoDB 表空间文件中的**物理偏移量标识符**
- 格式为：`空间ID + 页号`（如 `表空间ID:页号`）
...
- 每个非叶子节点条目格式：`[主键值, 子节点页号]`
- 例如：`[100, 0x00001234]` 表示主键值100对应的子节点位于页号0x1234

#### 步骤2：页号到物理位置的转换

1. **通过缓冲池(Buffer Pool)查找**：
...
2. **磁盘IO流程**：
   ...
   - 计算公式：`物理偏移量 = 页号 × 页大小(默认16KB)`
   - 例如页号0x1234 → 偏移量4660×16KB=76,165,120字节
