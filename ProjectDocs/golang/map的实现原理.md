...
![hashmap bmap](../images/golang/0.png)
...
![bmap struct](../images/golang/1.png)
...
Go 的 `map` 在写入时（如 `m[key] = value`）会调用底层函数 `mapassign`，其逻辑如下：

#### 步骤 1：计算哈希并定位桶

1. 计算 `key` 的哈希值 `hash`。
2. 用哈希低位确定桶位置：`bucketIndex = hash & (1<<B - 1)`。

#### 步骤 2：遍历桶（及溢出桶）

- **检查 `tophash` 数组**（哈希高 8 位）：
  - **如果 `tophash[i]` 匹配当前 key 的哈希高 8 位**：
    - 进一步比较 `key` 是否相等（`keys[i] == key`）。
    - **如果 `key` 匹配**：直接更新 `values[i]`（**覆盖旧值**）。
    - **如果 `key` 不匹配**（哈希冲突）：继续检查下一个槽位。
  - **如果 `tophash[i] == 0`（空槽）**：
    - **记录第一个空槽的位置**（但不会立即插入，需先确认 `key` 是否已存在）。
  - **如果 `tophash[i] == 1`（逻辑删除标记）**：
    - 跳过，不占用此位置（除非扩容时重整）。

#### 步骤 3：决定写入位置

- **如果 `key` 已存在**：更新对应 `value`。
- **如果 `key` 不存在**：
  - **优先复用第一个遇到的空槽**（步骤 2 中记录的）。
  - 如果桶已满（无空槽），分配溢出桶并插入。
